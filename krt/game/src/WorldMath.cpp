    ...
    Simple_Polynome& polyOut)
{
	...
	if (mod_point_zu12 != 0 && mod_point_zu23 != 0)
	{
		...
		if (major_divide != 0)
		{
			...
			polyOut.const_off =
			    ((block_trans(po_diff, u, 1, 2) / mod_point_zu12 - block_trans(po_diff, u, 2, 3) / mod_point_zu23)) / major_divide;
	...
	if (mod_point_zu23 != 0 && mod_point_zu13 != 0)
	{
		...
		if (major_divide != 0)
		{
			...
			polyOut.const_off =
			    ((block_trans(po_diff, u, 2, 3) / mod_point_zu23 - block_trans(po_diff, u, 1, 3) / mod_point_zu13)) / major_divide;
	...
	if (mod_point_zu12 != 0 && mod_point_zu13 != 0)
	{
		...
		if (major_divide != 0)
		{
			...
			polyOut.const_off =
			    ((block_trans(po_diff, u, 1, 2) / mod_point_zu12 - block_trans(po_diff, u, 1, 3) / mod_point_zu13)) / major_divide;
		...
		if (ivec(z, datIndex) != 0)
		{
			...
			if (mod_point_zu != 0)
			{
				...
				if (major_divide != 0)
				{
					...
					polyOut.const_off =
					    ((ivec(po_diff, datIndex) / ivec(z, datIndex) - block_trans(po_diff, u, prim_divide, sec_divide) / mod_point_zu)) / major_divide;
...
inline Quader::Quader(
    ...
    rw::V3d trl, rw::V3d trr, rw::V3d tfl, rw::V3d tfr) : brl(brl), brr(brr), bfl(bfl), bfr(bfr),
...
inline bool intersectQuaderPlaneWithQuader(
    ...
    const Quader& right)
{
	return (isQuaderPlaneIntersectingPlane(bl, br, tl, tr, right.brl, right.brr, right.trl, right.trr) == true) || // rear plane
