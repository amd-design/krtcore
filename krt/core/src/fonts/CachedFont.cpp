...
ResultingGlyphRun* CachedFont::TargetGlyphRun(float originX, float originY, const DWRITE_GLYPH_RUN* glyphRun, const DWRITE_GLYPH_RUN_DESCRIPTION* glyphRunDescription, RGBA color)
{
	...
	else
	{
		...
		do 
		{
			...
			RGBA curColor = (colorGlyphRun->paletteIndex == 0xFFFF) ? color : RGBA::FromFloat(colorGlyphRun->runColor.r, colorGlyphRun->runColor.g, colorGlyphRun->runColor.b, colorGlyphRun->runColor.a);
