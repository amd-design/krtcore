...
const Rect& CachedFontPage::GetCharacterAddress(uint32_t codePoint)
{
	return m_characterAddresses[codePoint - m_minCodePoint].address;
}

const Rect& CachedFontPage::GetCharacterSize(uint32_t codePoint)
{
...
void CachedFontPage::CreateCharacter(uint32_t codePoint)
{
	...
	check(SUCCEEDED(g_fontRenderer.GetDWriteFactory()->CreateGlyphRunAnalysis(&dummyRun, 1.0f, nullptr, DWRITE_RENDERING_MODE_GDI_NATURAL, DWRITE_MEASURING_MODE_GDI_NATURAL, 0.0f, 0.0f, glyphRunAnalysis.GetAddressOf())));

	RECT textureBounds;
	glyphRunAnalysis->GetAlphaTextureBounds(DWRITE_TEXTURE_CLEARTYPE_3x1, &textureBounds);

	Rect boundsBits((float)textureBounds.left, (float)textureBounds.top, (float)textureBounds.right, (float)textureBounds.bottom);
